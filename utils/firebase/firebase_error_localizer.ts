function VerifyErrorCode(errorCode: String): String {
  switch (errorCode) {
    ...
    case 'auth/too-many-requests':
      return 'Suas ações estão sendo limitadas!';
...
function localizedFirebaseError(err: firebaseError): String {
  let localizedErrorMessage = VerifyErrorCode(err.code);
...
function handleFirebaseError(
  err: firebaseError,
  setreqErrorMsg: CallableFunction
) {
  const localizedErrorMessage = localizedFirebaseError(err);

  if (localizedErrorMessage !== null) {
    setreqErrorMsg(localizedErrorMessage);
  }
}
